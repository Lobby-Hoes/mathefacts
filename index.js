    ...
    fetch("./data.json")
        ...
        .then(data => {
            let insertData = data.data.map(({folge, name, faktthema, beschreibung, startzeit, endzeit, code}) => {
                ...
                let embed = "<iframe style=\"border-radius:12px\" src=\"https://open.spotify.com/embed/episode/" + code + "?utm_source=generator&t=" + dayjs(startzeit, "hh:mm:ss").diff(dayjs("00:00:00", "hh:mm:ss")) + "\" width=\"100%\" height=\"152 \" frameBorder=\"0\" allowfullscreen=\"\" allow=\"autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture\"></iframe>"
